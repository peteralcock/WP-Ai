...
tinyMCEPreInit.mceInit['elementorwpeditor'].plugins += ",aikit_classic";

tinyMCEPreInit.mceInit['elementorwpeditor'].toolbar1 = tinyMCEPreInit.mceInit['elementorwpeditor'].toolbar1 || "";
tinyMCEPreInit.mceInit['elementorwpeditor'].toolbar1 += ",aikit_classic_button_text";
tinyMCEPreInit.mceInit['elementorwpeditor'].toolbar1 += ",aikit_classic_button_images";

tinyMCEPreInit.qtInit['elementorwpeditor'].buttons = tinyMCEPreInit.qtInit['elementorwpeditor'].buttons || "";
tinyMCEPreInit.qtInit['elementorwpeditor'].buttons += ",aikit_classic_button_text";
tinyMCEPreInit.qtInit['elementorwpeditor'].buttons += ",aikit_classic_button_images";

jQuery( document ).ready(function() {

    jQuery( document ).on( 'DOMNodeInserted', '.mce-tinymce', async function (obj) {

        // if the closets .elementor-controls doesn't have a child with class aikit-editor-is-there then return
        if ( jQuery(obj.currentTarget).closest('#elementor-controls').find('.aikit-editor-is-there').length === 0 ) {
            return;
        }

        const rgb = jQuery('.mce-i-bold').css('color');
        // convert rgb to array
        const rgbArray = rgb.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/);

        const color = new Color(rgbArray[1], rgbArray[2], rgbArray[3]);
        const solver = new Solver(color);
        const result = solver.solve();

        // set the filter for any i inside a mce-aikit-classic-main-button
        jQuery('.mce-aikit-classic-main-button i.mce-ico').css('filter', result.filter);

        jQuery('.mce-floatpanel').css('width', 'auto !important')
        jQuery('.mce-container-body').css('width', 'auto !important')

    } );

    elementor.hooks.addAction( 'panel/open_editor/widget/aikit_editor', function( panel, model, view ) {
...
jQuery( document ).ready(function() {
    ...
    tinymce.PluginManager.add('aikit_classic', function(editor, url) {
        ...
        if ( jQuery('#aikit-js-configs').length === 0 ) {
            return;
        }

        let aikit = JSON.parse(jQuery('#aikit-js-configs').val());
        ...
        for (let size in imageGenerationOptions.sizes) {
            ...
            for (let count of imageGenerationOptions.counts) {
                console.log(aikit.pluginUrl + 'includes/icons/image-' + size + '.svg')
                imageMenu.push({
