...
namespace AIKit\Dependencies\Faker;

use AIKit\Dependencies\Faker\Container\ContainerInterface;
...
class Generator
{
    ...
    public function ext(string $id): Extension\Extension
    {
        if (!$this->container->has($id)) {
            throw new Extension\ExtensionNotFound(sprintf(
                'No AIKit\Dependencies\Faker extension with id "%s" was loaded.',
                $id,
            ));
        }
    ...
    public function getFormatter($format)
    {
        ...
        // "AIKit\Dependencies\Faker\Core\Barcode->ean13"
